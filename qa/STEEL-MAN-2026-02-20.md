...
**Overall Readiness:** Ready with Caveats — critical integration bugs fixed (S0-1, S1-1, S1-2, S1-4), one S1 remaining (error recovery), plus integration testing still needed.
...
### S1-3: No Error Recovery in Synthesis Pipeline

**Area:** EnhancedNotePreviewModal synthesis flow
**File:** `EnhancedNotePreviewModal.tsx`

**Problem:** If the synthesis API call fails (network error, OpenAI outage, token limit exceeded), the user sees an error state but has no way to retry or fall back to the local merge. The local merge runs first (on modal open), then synthesis replaces it. If synthesis fails, the local merge output should remain visible — but the error handling UX needs verification. GPT-5.2 is an expensive model ($10/M output tokens); if the input context is very large (long visit transcript + chart prep + manual notes + scales + imaging + recommendations), it could hit token limits.

**Impact:** Clinician is stuck in the modal with no usable note output after a synthesis failure.

**Recommendation:** Add explicit error recovery: show the local merge as fallback, add a "Retry Synthesis" button, and add input token estimation with a warning if the context is approaching limits.
...
### Before Launch (S1)
1. **Verify chart prep data flow** — trace `onChartPrepComplete` → `chartPrepOutput` → modal → synthesis API
2. **Strip markers before synthesis** — remove `--- Visit AI ---` and `--- Chart Prep ---` markers from `manualData` before building synthesis context
3. **Add synthesis error recovery** — show local merge as fallback, add retry button
4. **Fix note review section IDs** — include valid ID list in the review prompt
