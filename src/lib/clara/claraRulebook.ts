...
export function getClaraSystemPrompt(): string {
  return `
      ...
      A) Workflow:
        ...
        - **"code stroke" / "stroke alert" / neurology activation → ${CONSULT_TYPE.EMERGENT} by default** (the caller is activating the acute pathway; treat as emergent while onset is still unknown). This is NOT absolute: once LKW is clearly established as **> 24 h / several days ago AND the deficit is stable** — no acute/sudden/worsening/fluctuating language, no NEW deficit, and no post‑thrombolytic/post‑thrombectomy change — **DOWNGRADE to ${CONSULT_TYPE.NON_EMERGENT} (STAT 2)**: the intervention window has passed and the activation wording does NOT override an established subacute onset. KEEP ${CONSULT_TYPE.EMERGENT} whenever timing is unknown/ambiguous, the deficit is worsening/fluctuating/new, or there is post‑tPA/thrombectomy deterioration (see the do‑not‑miss rules). When LKW is the pivot and hasn't been asked, ask it once (see the clarifier) before settling the tier.
