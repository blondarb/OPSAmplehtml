...
// Voice mode variant — same content but without JSON output format requirement
export function buildFollowUpVoicePrompt(context: PatientScenario): string {
  const basePrompt = buildFollowUpSystemPrompt(context)
  // Replace the OUTPUT FORMAT section for voice mode
  return basePrompt.replace(
    /## OUTPUT FORMAT[\s\S]*?(?=## WHAT YOU MUST NEVER DO)/,
    `## OUTPUT FORMAT
For voice mode, respond naturally in conversation. Do not use JSON formatting.
