 ...
 */
function resolveClientPreference(): { provider: VoiceProviderKind; explicit: boolean } {
  ...
  try {
    ...
    if (isValid(urlVal)) return { provider: urlVal, explicit: true }
  } catch {
  ...
  try {
    ...
    if (isValid(stored)) return { provider: stored, explicit: true }
  } catch {
  ...
  // NOT explicit. The caller must then OMIT `provider` from the session
  // request so the SERVER's VOICE_PROVIDER env var decides.
  //
  // 2026-08-06: this is the bug that made VOICE_PROVIDER=nova inert. The hook
  // always sent `provider`, and the route resolves
  // `body.provider ?? process.env.VOICE_PROVIDER ?? 'openai'` — so a
  // client-side fallback of 'openai' permanently shadowed the server default,
  // and the only way to reach Nova was an explicit ?voice=nova link.
  return { provider: 'openai', explicit: false }
}
...
export function useVoiceProviderPreference(): [
  ...
  // False until the user (or a ?voice= link) actually picks one. Callers send
  // `provider` to the session route ONLY when this is true.
  const [explicit, setExplicit] = useState(false)

  // Resolve the real preference once we're on the client.
  useEffect(() => {
    const resolved = resolveClientPreference()
    setProviderState(resolved.provider)
  ...
  return [provider, setProvider, explicit]
}
