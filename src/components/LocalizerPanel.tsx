  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        {/* Degraded-pipeline notice — a partial result must never look fully grounded */}
        {hasData && data.partial && (
          <div style={{
            display: 'flex',
            ...
            <span style={{ color: '#fbbf24', fontSize: '0.68rem', lineHeight: 1.45 }}>
              Partial analysis — {data.degradedReason?.toLowerCase().includes('kb')
                ? 'guideline evidence retrieval is unavailable; this differential has no source citations.'
                : (data.degradedReason ?? 'a pipeline step failed.')}
            </span>
          </div>
        )}
