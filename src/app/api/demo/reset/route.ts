...
export async function POST() {
  try {
    ...
    const { data: demoPatients } = await from('patients')
      .select('id')
      .in('mrn', DEMO_MRNS)
    ...
    const visitIdsToDelete = (visits || []).map((v: any) => v.id)

    // Step 2: Delete clinical notes for those visits
    if (visitIdsToDelete.length > 0) {
      const { error: noteDeleteError } = await from('clinical_notes')
        .delete()
      ...
      const { error: visitDeleteError } = await from('visits')
        .delete()
    ...
    const { error: appointmentResetError } = await from('appointments')
      .update({
        status: 'scheduled',
        visit_id: null,
    ...
    const { data: futureAppointments } = await from('appointments')
      .select('id')
      .in('patient_id', demoPatientIds)
      .gt('appointment_date', today)

    if (futureAppointments && futureAppointments.length > 0) {
      const { error: futureDeleteError } = await from('appointments')
        .delete()
        .in('id', futureAppointments.map((a: any) => a.id))
    ...
    if (demoPatientIds.length > 0) {
      const { data: dynamicPatients } = await from('patients')
        .select('id')
        .not('id', 'in', `(${demoPatientIds.join(',')})`)

      if (dynamicPatients && dynamicPatients.length > 0) {
        const dynamicIds = dynamicPatients.map((p: any) => p.id)

        // Delete their visits' clinical notes
        const { data: dynamicVisits } = await from('visits')
          .select('id')
          .in('patient_id', dynamicIds)

        if (dynamicVisits && dynamicVisits.length > 0) {
          const dynamicVisitIds = dynamicVisits.map((v: any) => v.id)
          await from('clinical_notes').delete().in('visit_id', dynamicVisitIds)
          ...
          await from('visits').delete().in('id', dynamicVisitIds)
        }

        // Delete their medications and allergies
        await from('patient_medications').delete().in('patient_id', dynamicIds)
        await from('patient_allergies').delete().in('patient_id', dynamicIds)

        // Delete their appointments
        await from('appointments').delete().in('patient_id', dynamicIds)

        // Delete their diagnoses
        await from('diagnoses').delete().in('patient_id', dynamicIds)

        // Delete their imaging studies
        await from('imaging_studies').delete().in('patient_id', dynamicIds)

        // Delete the patients themselves
        const { error: patientDeleteError } = await from('patients')
          .delete()
          .in('id', dynamicIds)
