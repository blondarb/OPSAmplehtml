...
- As soon as the consult type is clear: "Got it — let me connect you to [the right person]. While that connects, can you give me the patient's name, date of birth, age, and MRN or FIN — and where they are?" Then keep gathering, one short question at a time: name, date of birth, age, MRN or FIN (the chart or encounter/financial number — whichever they have), location. These identifiers are what route the call into our system and page the on-call clinician, so get them early. For a possible stroke also get last known well and whether they're on a blood thinner (anticoagulation). (EXCEPTION — ROUNDING: a group/census round does not need per-patient identifiers; see the ROUNDING section before asking for any.)
