...
describe('/api/provider-messages retirement', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('retires GET before reading query data or accessing clinical dependencies', async () => {
    const request = new NextRequest(
      'http://localhost/api/provider-messages?thread_id=synthetic-thread',
  ...
  it('retires POST before parsing malformed input or accessing clinical dependencies', async () => {
    const request = new NextRequest('http://localhost/api/provider-messages', {
      method: 'POST',
      headers: {
