...
import { buildHistorianSessionRequest } from '@/lib/historianSessionClient'
...
interface UseRealtimeSessionOptions {
   ...
   */
  consultId?: string
  /** Standalone correlation key for localizer and red-flag tracing only. */
  localizerSessionId?: string
  /**
...
export function useRealtimeSession(options: UseRealtimeSessionOptions): UseRealtimeSessionResult {
  ...
  const runLocalizer = useCallback(async () => {
    const localizerEnabled =
      options.enableLocalizer !== false &&
      sessionTypeRef.current !== 'referral_clarification'
    ...
    try {
      const res = await fetch('/api/ai/historian/localizer', {
        ...
        body: JSON.stringify({
          sessionId:
            options.consultId ?? options.localizerSessionId ?? 'ephemeral',
      ...
      console.warn(
        '[localizer] run failed (session continues):',
  ...
  const handleToolCall = useCallback((toolName: string, toolUseId: string, input: unknown) => {
    ...
    if (toolName === 'save_interview_output') {
      try {
        ...
        narrativeSummaryRef.current =
          typeof narrative_summary === 'string' ? narrative_summary : null
    ...
    if (toolName === 'save_scale_responses') {
      try {
        const scaleArgs = args as unknown as SaveScaleResponsesArgs
      ...
      ;(async () => {
        try {
          ...
          if (result?.done === true && typeof args.scale_id === 'string') {
            ...
            } as unknown as SaveScaleResponsesArgs)
          }
      ...
      case 'userTranscript': {
        ...
        if (userText.trim()) {
          ...
          const fullText = transcriptRef.current
            ...
            .map(t => t.text)
            .join(' ')
          const detection = detectRedFlags(
            fullText,
            options.consultId ?? options.localizerSessionId ?? 'ephemeral',
          )
  ...
  const startSession = useCallback(async () => {
    ...
    try {
      ...
      sessionTypeRef.current = resolvedSessionType ?? options.sessionType ?? 'new_patient'
      ...
      await provider.start({
        ...
        consultId: sessionConfig.consult_id ?? options.consultId,
      })
  ...
  const pushLocalizerContext = useCallback(
    (pushPayload: {
      ...
      if (sessionTypeRef.current === 'referral_clarification') return
      const provider = providerRef.current
      if (!provider) return
