...
- **Outpatient product ideas captured — Steve/Prachi strategy meeting (2026-06-19)**: Idea capture only (no code). Meeting-derived outpatient-product ideas recorded in [`docs/IDEAS_2026-06-19_prachi-meeting.md`](docs/IDEAS_2026-06-19_prachi-meeting.md) — refinements to the referral-source-driven **triage tool** (reads referral notes + applies Sevaro's urgency algorithm to advise schedulers on timing, specialist, and workup location) and the **AI Historian** (OLD CARTS + auto-differential, EHR-populating, 15–20 min visits), plus strategy-deck restructuring notes and BD pipeline (Marshall Medical, Kaiser). A CEO/CMO-facing one-pager of the outpatient stack lives at `docs/Sevaro_Outpatient_OnePager_v0.1.docx` (+ `.pdf`). Maps to the `/triage` and `/consult` routes.
