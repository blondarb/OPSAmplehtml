...
- **Acoustic speech biomarkers — multi-engine bake-off harness + Praat sidecar auth (PRs #129, #131, Jul 1-2)** — Phase A scaffold shipped: pure-TS engine + `/api/ai/voice-biomarkers` + `scoreAllEngines()` parallel fan-out with `parselmouthEngine` HTTP adapter (`VOICE_PRAAT_URL` sidecar contract). x-api-key forwarded to Praat sidecar for PHI-trial audio auth (PR #131). Bake-off harness ready; threshold tuning + MDS-UPDRS mapping next. Spec: `docs/plans/2026-06-30-acoustic-speech-biomarkers-spec.md`.
- **Historian background-noise fix (2026-06-30)** — Added `audio.input.noise_reduction` to the Realtime session (`src/app/api/ai/historian/session/route.ts` + `getNoiseReductionConfig()` in `historianTypes.ts`), default `far_field`; OpenAI filters audio before VAD, cutting false positive turn-taking from speakerphone/open-room noise. Fail-open: route retries once without the field if OpenAI rejects it.
- **Scale results linked to patient for longitudinal tracking (2026-06-30)** — Added `resolveScalePatientId()` to both historian scale-write paths (`/api/ai/historian/scales` submit + step); looks up `patient_id` from `historian_sessions` then `neurology_consults`; fail-safe → null, never throws. Fixes `scale_results` landing with no patient linkage (0/5 rows had `patient_id`).
