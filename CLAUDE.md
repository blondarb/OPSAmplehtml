...
- **SDNE Facial Asymmetry Index thresholds (2026-06-04)**: Defined screening thresholds for the Facial domain (Task T07) AsI metric. Added `FACIAL_ASYMMETRY_AsI` constant (`greenMax: 0.08`, `yellowMax: 0.15`) and `classifyFacialAsymmetry()` helper to `src/lib/sdneTypes.ts`, documenting the Vrochidou asymmetry-index formula (mean of per-pair-normalized `|d_R−d_L|/(d_R+d_L)` landmark deviation from midline; the SDNE headset consumes a denominator-free blendshape variant — see the SDNE repo's `docs/T07_FACIAL_ASYMMETRY_AsI.md`). Calibrated against our demo baselines and a hand-held CV AsI reference device (UT Arlington capstone, Bell's palsy cohort). `SDNEInterpretation.tsx` facial observation now derives severity from the measured AsI (broadened to surface YELLOW/hypomimia, not just RED), falling back to the domain flag when no index is present. Fixed a mock-data inconsistency where the resolving post-stroke session (`sdne-james-003`) carried the acute T07 value (0.28 → would read RED) despite its YELLOW "improved to 0.12" narrative. Documented formula, threshold table, and sensitivity caveat (subtle central facial weakness needs the upper-vs-lower-face ratio; XR-headset upper-face occlusion is a known limit) in `docs/PRD_SDNE_Integration.md` §2.4.
...
**Status**: Active — verified June 4, 2026
