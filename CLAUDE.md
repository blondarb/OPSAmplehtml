...
- **Acoustic speech-biomarker scoping spec (2026-06-30)**: Reviewed `menelly/AI_Ears` (an MCP server that analyzes audio *beyond* transcription — voice tremor, breath, dynamics). It surfaced a capability we lack: every voice surface today (Historian/Intake/Follow-Up/dictation/visit) uses audio only for ASR and **discards the waveform**, throwing away clinically measurable neuro speech signs (hypophonia, monopitch, vocal tremor, DDK rate, jitter/shimmer/HNR → MDS-UPDRS speech item / Frenchay). Wrote a scoping spec at `docs/plans/2026-06-30-acoustic-speech-biomarkers-spec.md`: a Sevaro-side **testable MVP** (browser `VoiceTaskCapture` for sustained-/a/ + Pa-Ta-Ka + reading, `POST /api/ai/voice-biomarkers` pure-TS engine behind `VOICE_BIOMARKERS_ENABLED`, GREEN/YELLOW/RED card reusing `SDNEFlagChip`, persisted to `scale_results`) plus an **engine bake-off** for test week (Parselmouth/Praat, openSMILE eGeMAPS, DisVoice vs pure-TS) on labeled profiles (James Morrison=PD, Eleanor Wright=ET, Maria Santos=normal). Key stack constraint: Amplify SSR is a Node Lambda (no Python) — a Praat-grade engine needs a separate Python sidecar, and the bake-off's go/no-go decides whether that infra is warranted. SDNE already runs Pa-Ta-Ka DDK (T08, `sdneTypes.ts`) on external XR — full acoustic scoring at that head-mic source is a hand-off to the SDNE team (path 2). Proposed-only; no code shipped.
...
### Planned
- Acoustic speech biomarkers (🎯 test-week bake-off) — measure neuro speech signs from captured audio instead of discarding the waveform after ASR; Sevaro-side TS MVP behind `VOICE_BIOMARKERS_ENABLED` + engine comparison (Parselmouth/eGeMAPS/DisVoice vs pure-TS) on labeled profiles, mapped to MDS-UPDRS speech item. Spec: `docs/plans/2026-06-30-acoustic-speech-biomarkers-spec.md`
