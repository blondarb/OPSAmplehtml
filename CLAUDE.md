...
- **Nova Sonic allows SYSTEM content ONLY ONCE per prompt** (the init system prompt). A second SYSTEM content block fails the whole stream with `Duplicate SYSTEM content`. All mid-conversation text injection (localizer pushes, scale instructions, early-end nudges) must go through `userText()` (role USER), NOT `systemContent()`. The relay's `pushSystemText()` already does this — do not switch it back to SYSTEM. (OpenAI Realtime allowed re-sending system context via `session.update`; Nova does not.)

## Recent Changes (Summary)

- **Historian conversational-quality tuning from live testing (2026-06-05)**: Demo-only, from a real `/consult` voice run. Three `historianPrompts.ts` refinements + one client change: (1) one/two questions per turn MAX, never 3+ (model was stacking questions); (2) NEVER name/hint a diagnosis to the patient — strengthened rule 4 to forbid condition names and clarify the Localizer differential is physician-only (the model had verbalized differentials, which on Nova leak in because the localizer push arrives as a USER turn); (3) Phase-2 now tells the model to probe the differential MORE deeply but only via plain symptom questions. (4) `useRealtimeSession.pushLocalizerContext` wraps the localizer delta in an explicit "[INTERNAL CLINICAL NOTE — do not speak aloud / name to patient]" frame so the USER-turn injection isn't read out. No schema/flow change.

- **Historian plain-language opening + Nova error visibility (2026-06-05)**: Demo-only. (1) The AI Historian was reading the clinical referral back to patients verbatim (e.g. "evaluation of episodic migraine with aura for consideration of CGRP therapy"). Tightened `historianPrompts.ts`: core rule now mandates plain, ~8th-grade conversational language and "never read clinical/referral text back verbatim", and the referral-anchored opening now instructs the model to *translate* the reason into everyday words (with an explicit good/bad example) instead of reciting it. Provider-agnostic (applies to Nova + OpenAI). (2) Nova voice sessions were dying with an unreadable `Voice provider error: [object Object]` because Bedrock's `modelStreamErrorException`/`internalServerException` arrive as plain objects and the relay did `String(err)`. The relay (`services/nova-sonic-relay/src/server.ts`) now extracts a real message (`name: message`/JSON fallback) AND logs the full error server-side — which surfaced the actual cause of the mid-interview freeze: **`Duplicate SYSTEM content. SYSTEM content can only be provided once per prompt.`** (3) FIXED that freeze: the relay's `pushSystemText` (localizer pushes every 3 turns, scale injection, early-end flush) reused `systemContent()` (role SYSTEM), and Nova permits SYSTEM content only once per prompt — so the first mid-conversation injection (the scale step) killed the stream. Added a `userText()` builder (role USER) and routed `pushSystemText` through it; Nova accepts repeated USER text turns. OpenAI Realtime tolerated re-sent system context via `session.update`; Nova does not. Regression-tested in `eventBuilders.test.ts`.
