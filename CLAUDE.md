...
### Recent
- **Triage SSE streaming — clear Amplify ~28s gateway timeout (PR #110, Apr 29)** — Switched `/api/triage` and `/api/triage/extract` from buffered `NextResponse.json` to streaming `text/event-stream` SSE responses; new `invokeBedrockJSONStreaming` in `src/lib/bedrock.ts`; `streamPostJSON`/`streamPostFormData` helpers in `src/lib/triage/streamClient.ts`; 5s heartbeat keeps connection alive; `maxDuration` raised to 120s. Client call sites at `src/app/triage/page.tsx` (4) and `src/components/consult/TriageStepPanel.tsx` (1) stay structurally unchanged. Input validation still returns plain JSON 400s.
