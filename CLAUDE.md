...
- **ASR vocabulary biasing implemented (2026-06-07)**: New `src/lib/asr/clinical-lexicon.ts` biases all four voice/transcription surfaces toward a neurology lexicon (curated symptom/anatomy/scale terms interleaved with the full `NEURO_FORMULARY` brand+generic drug list). Realtime surfaces (Historian, Intake, Follow-Up) get a budget-trimmed Whisper `prompt`; dictation (`/api/ai/transcribe`) gets Deepgram Nova-3 `keyterm`. Follow-Up hoists the patient's own name/provider/meds first so they survive the ~224-token Whisper cap. Gated by `ASR_VOCAB_BIASING` (default on; `false`/`0`/`off` reverts without a deploy). Unit tests in `src/lib/__tests__/clinical-lexicon.test.ts` (13 cases). Spec updated to Implemented: `docs/plans/2026-06-07-asr-vocabulary-biasing-spec.md`.
