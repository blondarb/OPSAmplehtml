...
tmp_dir="$(mktemp -d /tmp/triage-migration-050-behavior-XXXXXX)"
...
# Apply the creating migration, then the extraction-specific tenant addition
# from 048. This exercises 050 against the real 045 table shape without
# needing unrelated consult/appointment fixtures required by all of 048.
psql "${psql_args[@]}" <<'SQL'
...
INSERT INTO triage_extractions (
  id, status, text_input, extracted_summary, created_at
) VALUES (
  '05000000-0000-4000-8000-000000000001',
  'complete',
  'Synthetic legacy referral.',
...
psql "${psql_args[@]}" \
  -f "$repo_root/tests/triage/migrations/050_triage_long_packet_provenance.behavior.sql"
