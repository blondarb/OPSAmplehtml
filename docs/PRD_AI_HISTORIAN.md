...
- **Localizer latency:** Step 3 runs a 300-token compact steer alongside 900-token clinician detail within the unchanged 15 s budget; completed detail preserves the legacy response, while a completed steer supplies a partial fallback and registry-validated scale id when detail fails (content-free `localizer_timing` telemetry).
